...
void DrawScene(glm::mat4 & mv_mat4)
{
	...
	//circle
	{
		...
		// draw circle
		glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, glm::value_ptr(color_blue));
		mesh_draw_arrays(mesh_circle);

		//restore model-view matrix (SCALE)
		glPopMatrix();
	}
	...
	//cube
	{
		...
		glColor3fv(color_lightGreen);
		mesh_draw_arrays(mesh_cube);
		glPopMatrix();

		//restore model-view matrix (position,rotation,scale...)
